...
const Chatbot: React.FC<ChatbotProps> = ({
  ...
  // Effect to handle the initial message from the landing page
  useEffect(() => {
    if (initialMessage && activeSession && !initialMessageSent.current) {
      initialMessageSent.current = true; // Prevents re-sending on re-renders
  ...
  const handleSendMessage = async (messageText: string | React.FormEvent) => {
    ...
    const textToSend =
      typeof messageText === "string" ? messageText : inputValue;
    ...
    try {
      ...
      const result: GeminiResponse = await getAiResponse(
        newMessages,
        courses,
        locale
      );
      ...
      const errorText =
        locale === "vi"
          ? "Xin lỗi, tôi đã gặp lỗi khi xử lý yêu cầu của bạn. Vui lòng thử lại."
                ...
                <div
                  className={`px-4 py-3 rounded-2xl ${
                    msg.sender === "user"
                      ...
                      : "bg-ai-bubble dark:bg-gray-700 text-gray-700 dark:text-gray-200 rounded-bl-none"
                  }`}
                >
                ...
                <div className="grid grid-cols-1 gap-4 mt-2 w-full">
                  {msg.recommendedCourses.map(({ course, reasoning }) => (
                    <CourseCard
                      key={course.id}
                      course={course}
                      onClick={() => onSelectCourse(course, reasoning)}
